...
import com.liuge.TMShop.entity.CollectGoodsEntity;
...
public class CollectGoodsAdapter extends BaseAdapter {
    ...
    CollectGoodsEntity goods;

    public void setGoods(CollectGoodsEntity goods) {
        this.goods = goods;
        this.notifyDataSetChanged();
    }

    public CollectGoodsAdapter(Context mContext) {
    ...
    public View getView(final int i, View view, ViewGroup viewGroup) {
        ViewHolder vh = null;
        if (view == null) {
            view = LayoutInflater.from(mContext).inflate(R.layout.item_layout_collect_goods, null);
        ...
        Glide.with(mContext).load(goods.getData().getResult().get(i).getDefault_image()).into(vh.iv_xianshi_img_1);
        vh.tv_xianshi_name_1.setText(goods.getData().getResult().get(i).getGoods_name());
        vh.tv_xianshi_detail_1.setText(goods.getData().getResult().get(i).getSpec());
        vh.tv_xianshi_price_1.setText(goods.getData().getResult().get(i).getPrice());
